...
def img_agu_channel_same(img_): # 将输入图像的所有通道设置为灰度值
...
class LoadImagesAndLabels(Dataset):
            ...
            if ".jpg" in f_:
                ...
                if len(hand_dict_)>0:
                    for msg in hand_dict_:
                        ...
                        hand_anno_list.append((bbox,pts)) # 将上面两个信息保存在hand_anno_list变量中
                        idx += 1
    ...
    def __getitem__(self, index):
        ...
        if random.random() > 0.55: # 这段代码的作用是对图像中的手部区域进行随机旋转、缩放、镜像等操作，以增加数据的多样性，最后归一化标记点为pts_tor_landmarks_norm
            offset_x = int((x_max-x_min)/8)
            offset_y = int((y_max-y_min)/8)
            ...
            if self.vis:
                ...
                cv2.waitKey(1)
        #-------------------------------------
        if self.flag_agu == True: # 训练数据生成器是否进行数据扩增
            if random.random() > 0.5: # 这段代码中的逻辑是在随机概率大于0.5时执行对比度增强操作
                c = float(random.randint(80,120))/100. # 对比度的缩放系数
                b = random.randint(-10,10) # 亮度的偏移量
                img_ = contrast_img(img_, c, b)
        if self.flag_agu == True:
            if random.random() > 0.9: # 随机概率大于0.9时执行色调增强操作
                # print('agu hue ')
                img_hsv=cv2.cvtColor(img_,cv2.COLOR_BGR2HSV)
                hue_x = random.randint(-10,10)
                # print(cc)
                img_hsv[:,:,0]=(img_hsv[:,:,0]+hue_x) # 将图像的色调通道（H通道）加上偏移量 hue_x，以实现色调的变化。
                img_hsv[:,:,0] =np.maximum(img_hsv[:,:,0],0)
                img_hsv[:,:,0] =np.minimum(img_hsv[:,:,0],180)#范围 0 ~180
                img_=cv2.cvtColor(img_hsv,cv2.COLOR_HSV2BGR)
        if self.flag_agu == True:
            if random.random() > 0.95: # 改为灰度图像
        ...
        img_ = (img_-128.)/256. # 将图像的像素值范围从 [0, 255] 缩放到 [-0.5, 0.5] 之间,相当于归一化
        img_ = img_.transpose(2, 0, 1) # 将图像的通道维度从 (H, W, C) 调整为 (C, H, W)，这是因为在 PyTorch 中通常采用 (C, H, W) 的顺序表示图像数据。


        pts_tor_landmarks_norm = np.array(pts_tor_landmarks_norm).ravel() # 关键点坐标展平成一个一维数组
