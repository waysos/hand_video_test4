...
def trainer(ops,f_log):
    try:
        ...
        if os.access(ops.fintune_model,os.F_OK):# checkpoint
            chkpt = torch.load(ops.fintune_model, map_location=device) # 将预训练模型参数放到chkpt
        ...
        for epoch in range(0, ops.epochs):
            ...
            if loss_mean!=0.:
                if best_loss > (loss_mean/loss_idx):
                    flag_change_lr_cnt = 0 # 用于记录损失连续下降的次数
                    best_loss = (loss_mean/loss_idx)
                else:
                    flag_change_lr_cnt += 1

                    if flag_change_lr_cnt > 50: # 调整学习率
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_path', type=str,
                        default="D:/hand_data/handpose_datasets_v2-2022-04-16/handpose_datasets_v2/",
    ...
    parser.add_argument('--fintune_model', type=str, default = 'D:/hand_data/handpose_x_model/ReXNetV1-size-256-loss-wing_loss102-0.115-20211108.pth',
        help = 'fintune_model') # fintune model 预训练模型
